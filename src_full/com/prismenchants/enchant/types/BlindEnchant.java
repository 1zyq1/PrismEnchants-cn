...
public class BlindEnchant
extends CustomEnchant {
    public BlindEnchant() {
        super("blind", "\u81f4\u76f2", Rarity.RARE, 8, ItemCategory.MELEE);
    }

    @Override
    public List<String> description(int n) {
        return List.of("&7" + (15 + n * 10) + "% \u51e0\u7387\u4f7f\u76ee\u6807 &8\u81f4\u76f2&7\u3002");
    }

    @Override
    public void onAttack(Player player, LivingEntity livingEntity, int n, EntityDamageByEntityEvent entityDamageByEntityEvent) {
        if (ThreadLocalRandom.current().nextInt(100) < 15 + n * 10) {
            livingEntity.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 40 + n * 20, 0));
        }
    }
}
